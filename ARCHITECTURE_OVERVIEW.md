...
Missiles require explicit **E** input after capsule unlock. Each Missile capsule adds one Player-owned round up to 12; empty clips reload in a fixed 12 seconds. Ballistic projectiles remain straight without their owner's valid lock and steer at `0.3` of missile turn strength with that lock. Missiles retain reference strength `1.0` and their owner-lock-first fallback behavior. The HUD derives expanding Speed cubes, gray ammunition cubes, and two-decimal reload countdowns from Player state and owns no gameplay timer.
