...
`Player.fire()` → generic `Projectile` → canonical `Game.projectiles` → projectile update and Sector 1 wall resolution → `Game.checkCollisions()` → authoritative damage/reward/death. Ballistic projectiles (Base Gun, Double, Antigun) and distinct Orb projectiles read only `owner.lockedAimTarget` and steer at the shared `MISSILE_HOMING_TURN_RATE × 0.3` when it remains valid; Laser and Missile remain separate families. HUD ammo/reload and expanding Speed cubes are derived every frame from Player-owned state.
